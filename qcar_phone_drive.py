# qcar_phone_drive.py
# Phone-based dual-joystick control for physical QCar (and Virtual Lab).
...
#
# Notes:
# - readmode 0 = immediate I/O (works well for hardware & Virtual Lab).
# - Use ARM to enable motion. DISARM stops & holds. E-STOP forces 0 commands.
...
<title>QCar Controller — Phone</title>
<style>
  :root{
  ...
  .hdr{display:flex;align-items:center;justify-content:space-between;background:linear-gradient(180deg,#0f1520,#0a1017);
  ...
  .pad-title{position:absolute; top:10px; left:10px;background:rgba(0,0,0,.25); border:1px solid var(--border); color:var(--fg);
    font-weight:800; font-size:13px; padding:6px 10px; border-radius:10px; letter-spacing:.3px; backdrop-filter: blur(2px);}
  .pad-instr{position:absolute; left:50%; bottom:10px; transform:translateX(-50%); color:var(--mut); font-size:12px; text-align:center; width:90%;
    background:rgba(0,0,0,.15); padding:6px 10px; border:1px dashed var(--border); border-radius:10px;}
  .stick{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:38%;aspect-ratio:1;border-radius:50%;
    background:#18202b;border:2px solid var(--accent);box-shadow:0 0 0 2px rgba(77,163,255,0.15), inset 0 0 12px rgba(0,0,0,0.35);
    display:block; overflow:hidden; font-size:clamp(28px, 7.5vmin, 64px);}
  .stick-hint{position:absolute;inset:0;pointer-events:none;color:#d9ecff;opacity:0.95;text-shadow:0 2px 2px rgba(0,0,0,.35); font-weight:900; line-height:1}
  ...
  button{background:var(--idle); color:var(--fg); border:1px solid var(--idle-br); border-radius:10px; padding:10px 14px; font-weight:700;
    display:inline-flex; align-items:center; gap:8px; transition:transform .04s, box-shadow .1s, background .15s, border-color .15s, color .15s, opacity .15s;}
  ...
  .rotate{position:fixed; inset:0; display:none; align-items:center; justify-content:center; background:rgba(10,13,18,.95);
    color:#e8eef6; text-align:center; z-index:9999; padding:24px;}
  .rotate .box{border:1px solid var(--border); border-radius:14px; padding:16px 18px; background:#0e1726; max-width:520px;}
  .rotate h2{margin:0 0 8px 0; font-size:18px}
  .rotate p{margin:0; color:var(--mut); font-size:14px}
  @media (orientation: portrait) {.hdr,.row,.hud,.footer{display:none;} .rotate{display:flex;}}
  @media (orientation: landscape){.hdr,.row,.hud{display:flex;} .hud{display:grid;} .rotate{display:none;}}
</style>
</head>
<body>
<div class="wrap">
  <div class="hdr">
    <div class="brand">
      <div class="logo" aria-hidden="true"><span>Q</span></div>
      <div><div class="title">QCar Controller</div><div class="subtitle">Phone Joysticks</div></div>
    </div>
    <div class="hdr-rights"><div class="hdr-chip" id="hdrInfo">Connecting…</div></div>
  </div>

  <div class="rotate"><div class="box"><h2>Please rotate your phone</h2><p>This controller is optimized for <b>landscape</b>.</p></div></div>

  <div class="row">
    <div class="pad" id="padLeft" aria-label="Steering">
      <div class="pad-title">Steering (X-axis)</div>
      <div class="stick" id="stickLeft"><div class="stick-hint"><span class="u">↑</span><span class="d">↓</span><span class="l">←</span><span class="r">→</span></div></div>
      <div class="pad-instr">Slide <b>left/right</b> to steer.</div>
    </div>

    <div class="pad" id="padRight" aria-label="Throttle">
      <div class="pad-title">Speed / Throttle (Y-axis)</div>
      <div class="stick" id="stickRight"><div class="stick-hint"><span class="u">↑</span><span class="d">↓</span><span class="l">←</span><span class="r">→</span></div></div>
  ...
  <div class="hud">
    <div class="card">
      ...
      <div>
        <label>Steering Gain (rad): <span id="steerGainVal">0.50</span></label>
      ...
      <div class="mut" style="margin-top:6px">Left = steer (X). Right = throttle (Y).</div>
    </div>
    <div class="card">
      ...
      <div class="mut" style="margin-top:8px">Logging on robot host.</div>
    </div>
  </div>

  <div class="footer">© Phone Controller</div>
</div>
...
(function(){
  ...
  function renderArmState(armed, estop){
    uiState.armed = !!armed; uiState.estop = !!estop;
  ...
  ws.onmessage = (ev)=>{
    try{
      ...
      if (msg.type==='telemetry'){
        ...
        if ('armed' in msg || 'estop' in msg){ renderArmState(!!msg.armed, !!msg.estop); }
      }
    }catch(_){}
  }
...
class ControllerState:
    ...
    def compute(self, prev_throttle, prev_steering):
        ...
        lx = self._deadzone(float(self.left.get('x',0.0)), dz)   # [-1,1], right +
        ry = self._deadzone(float(self.right.get('y',0.0)), dz)  # [-1,1], up +

        # Map sticks to car:
        steering_cmd = -steer_k * lx           # invert so right = right turn
        throttle_cmd =  vmax   * ry            # m/s

        # 1st-order smoothing (EMA)
...
async def controller_task(sample_rate: float, log_path: str, read_mode: int):
    ...
    try:
        while True:
            ...
            linearSpeed = float(myCar.motorTach)  # m/s
        ...
        myCar.terminate()
        print("[QCar] Control loop stopped.")
